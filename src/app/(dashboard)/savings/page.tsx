...
export default function SavingsPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      // Pass isSavings: true
      const data = await getFinanceSummary(true);
      setSummary(data);
    } catch (error) {
      toast.error("Kumbara verileri yüklenirken bir hata oluştu.");
    } finally {
      setIsLoading(false);
    }
  }, []);
  ...
  return (
    <div className="space-y-10 animate-chat-fade pb-32">
      <DashboardHeader
        title="Kumbara"
        subtitle="Hayallerin için biriktirdiklerin."
        showSearch={false}
      />
        ...
        <div className="lg:col-span-8 bg-[#111827] border border-[#1f2937] rounded-3xl overflow-hidden shadow-sm flex flex-col h-full min-h-[600px]">
          <div className="p-8 border-b border-[#1f2937] space-y-6">
            <div className="flex items-center justify-between">
              <h3 className="text-xl font-bold text-[#e5e7eb]">Kumbara Geçmişi</h3>
          ...
          <div className="flex-1 overflow-y-auto no-scrollbar">
            {isLoading ? (
              ...
              <div className="p-20 text-center flex flex-col items-center gap-4">
                ...
                <p className="text-sm font-bold text-[#9ca3af]/40 italic">Kumbara boş gözüküyor.</p>
              </div>
            ) : (
              ...
              <div className="relative z-10 text-[#e5e7eb]">
                ...
                <div className="space-y-10">
                  <div className="space-y-2">
                    <p className="text-[11px] font-bold text-[#9ca3af] uppercase tracking-[0.2em] opacity-40">Kumbara İşlemi</p>
